...
export function makeMetalRobot() {
  ...
  metalRobot.add(legL);
  //  胴体の作成
  const bodyGeometory = new THREE.BoxGeometry(bodyW - bodyD, bodyH, bodyD);
  const body = new THREE.Group;
  body.add( new THREE.Mesh(bodyGeometory, metalMaterial));
...
cardboardRobot.add(legL);
  ...
  //  胴体の作成
  const bodyGeometory = new THREE.BoxGeometry(bodyW, bodyH, bodyD);
  const body = new THREE.Group;
  body.add( new THREE.Mesh(bodyGeometory, cardboardMaterial));
  body.position.y = legLen + bodyH/2
